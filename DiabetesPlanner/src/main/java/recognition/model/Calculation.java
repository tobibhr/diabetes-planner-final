...
/**
 * This static class contains the methods needed to preprocess, label and aggregate the sensor data. After aggregation, a labeled record is written into an SQLite Database {@link #writetoDatabase(MySQLiteHelper, String[])}
...
public class Calculation extends Activity {
	 ...
	 * @author Robert and Mats
	 */
	static double [][] preprocessRecords (double [][] raw){
		...
		recordsProcessed = new double [P.arraySize][13];
		...
		recordsforStatistics = new double [3] [P.initTimeWindow * P.sampleRate];
		
		
		//for statement for all 120 time windows to be calculated
		for(int i=0; i<P.arraySize; i++){ //30
			...
			for (int ii = 0; ii <3; ii++){
				...
				for (int iii=0; iii<(P.initTimeWindow * P.sampleRate); iii++){ //60
				
					//new time window every 2.5 seconds --> every 125th record of the raw data (1*10)
					recordsforStatistics[ii][iii] = (double) raw[((i*P.overlapWindow*P.sampleRate)+iii)][ii];	//10										
				}	
			...
			//add TimeStamp
			recordsProcessed[i][12]=raw[i*P.overlapWindow*P.sampleRate][3];
			
//			Log.d("Message", "Mean of record number " + i + " for X is " + recordsProcessed[i][0]);
//			Log.d("Message", "StdDev of record number " + i + " for X is " + recordsProcessed[i][1]);
//			Log.d("Message", "Intqr of record number " + i + " for X is " + recordsProcessed[i][2]);
//			Log.d("Message", "Mean of record number " + i + " for Y is " + recordsProcessed[i][3]);
//			Log.d("Message", "StdDev of record number " + i + " for Y is " + recordsProcessed[i][4]);
//			Log.d("Message", "Intqr of record number " + i + " for Y is " + recordsProcessed[i][5]);
//			Log.d("Message", "Mean of record number " + i + " for Z is " + recordsProcessed[i][6]);
//			Log.d("Message", "StdDev of record number " + i + " for Z is " + recordsProcessed[i][7]);
//			Log.d("Message", "Intqr of record number " + i + " for Z is " + recordsProcessed[i][8]);
//			Log.d("Message", "Corr of record number " + i + " for X&Y is " + recordsProcessed[i][9]);
//			Log.d("Message", "Corr of record number " + i + " for X&Z is " + recordsProcessed[i][10]);
//			Log.d("Message", "Corr of record number " + i + " for Y&Z is " + recordsProcessed[i][11]);
//			Log.d("Message", "Timestamp of record number " + i + " is " + recordsProcessed[i][12]);
		}
	 ...
	 * @author Mats
	 */
	static String[][] labelRecords (double [][] processed, InputStream model, Context c) throws Exception {
		
		String[][] labeledRecords = new String [P.arraySize][2];
	 ...
	 * @author Robert
	 */
	static String [] aggregateRecords (String [][] labeled){
		String[] finalRecord = new String[2];
		String[] cache = new String[P.arraySize];
		...
		HashMap<String, Integer> wordCount = new HashMap<String, Integer>();
	       ...
	       Log.d("aggregateRecords", "During the " + P.finTimeWindow + " seconds starting at Millisecond " + finalRecord[0] + " the activity was " + finalRecord[1]);
